                        ...
                        match &data.dest_address_type {
                            AddressType::SubcmpSignal { cmp_address, .. } => {
                                ...
                                if indexes.len() <= 1 {
                                    ...
                                    for (i, instr) in indexes.iter().enumerate().skip(1) {
                                        instructions.push(get_local(producer.get_io_info_tag()));
                                        let offsetdim = 4 * i;
                                        ...
                                        let mut instructions_idxi = instr.produce_wasm(producer);
                ...
                if let AddressType::SubcmpSignal { .. } = data.dest_address_type {
                    ...
                    match &data.dest {
                        ...
                        LocationRule::Mapped { .. } => {
                            ...
                            instructions.push(call_indirect("$runsmap", "(type $_t_i32)"));
                        }
                    }
                    if producer.needs_comments() {
                ...
                let ((mut dest_prologue, dest_index), my_template_header) =
                    if let LocationRule::Indexed { location, template_header } = &data.dest {
                        ...
                        if !indexes.is_empty() {
                            ...
                            for (i, instr) in indexes.iter().enumerate().skip(1) {
                                let (mut index_code, index_exp) = instr.produce_c(producer);
                    ...
                    if self.is_parallel && data.dest_is_output {
                        if data.context.size > 0 {
                            ...
                            prologue.push(format!(
                                ...
                                CIRCOM_CALC_WIT, CTX_INDEX, dest_index
                            ));
                            prologue.push(format!(
